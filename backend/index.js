...
app.get("/api/getUserList", (req, res) => {
    ...
    const sqlSelectPassenger = "SELECT * FROM passenger"
...
app.post("/api/getStationListFull", (req, res) => {

    const sqlSelectPassenger = "SELECT * FROM station"
    db.query(sqlSelectPassenger, [], (err, result) => {
...
app.post('/api/updateClerk', (req, res) => {
    const { Clerk_ID, Name, Mobile, District, Password, Station_ID } = req.body;

    console.log({ Clerk_ID, Name, Mobile, District, Password, Station_ID });
    const updateClerk = "UPDATE booking_clerk SET Name= ? , Mobile = ?, Password = ?,Station_ID = ? WHERE Clerk_ID=?"; 
    db.query(updateClerk,[Name,Mobile,Password,Station_ID,Clerk_ID],(error,result)=>{
        if(error){
            console.log(error);
...
app.get('/api/trainList', (req, res) => {

    const trainList = "select * from train"; 
    db.query(trainList,(error,result)=>{
        if(error){
            console.log(error);
            return res.status(500).json({message:"internal error"});
        }
...
app.post('/api/updateStation', (req, res) => {
    const { District,Name,Station_ID} = req.body;

    console.log({ District,Name,Station_ID });
    const updateClerk = "UPDATE station SET Name= ?,District = ? WHERE Station_ID=?"; 
    db.query(updateClerk,[Name,District,Station_ID],(error,result)=>{
        if(error){
            console.log(error);
            return res.status(500).json({message:"internal error"});
        }
...
app.post('/api/toggleClerkStatus',(req,res)=>{
    const Clerk_ID = req.body.clerk_ID;
    const updateStatus = req.body.updateStatus;

    const statusSql = "UPDATE booking_clerk SET active_status=? where Clerk_ID =?";
    db.query(statusSql, [updateStatus, Clerk_ID], (err, result) => {
        if(err){
...
app.post('/api/toggletrainStatus',(req,res)=>{
    const {Train_ID,updateStatus} = req.body.Updatevalue
    const statusSql = "UPDATE train SET active_status=? where Train_ID =?";
    if(Train_ID === undefined || updateStatus === undefined ){
        return res.status(400).json({message:"Error updating"})
    }
    db.query(statusSql, [updateStatus, Train_ID], (err, result) => {
        if(err){
            res.status(500).json({ messge: 'updation Failed'})
            console.log(err)
        }
        // console.log(result)
        return res.status(200).json({message: "updated"});
...
app.post('/api/toggleUserStatus',(req,res)=>{
    const {passenger_id, updateStatus} = req.body.Updatevalue;

    if(passenger_id === undefined || updateStatus === undefined ) {
        console.log("Error");
        return res.status(400).json({message: "This Request cannot be Processed"})
    }

    const statusSql = "UPDATE passenger SET active_status=? where passenger_id=?";
    db.query(statusSql, [updateStatus, passenger_id], (err, result) => {
        if(err){
...
app.post('/api/toggleCoachStatus',(req,res)=>{
    console.log(req.body.updateValue);
    const {Train_ID,Coach_ID, updateStatus} = req.body.updateValue;
    ...
    const statusSql = "UPDATE train_coach SET coach_status=? where Train_ID=? AND Coach_ID=?";
    db.query(statusSql, [updateStatus, Train_ID,Coach_ID], (err, result) => {
        if(err){
            res.status(500).json({ message: err})
            console.log(err)
        }
...
app.post('/api/UpdateCoach', (req, res) => {
    const { selectedCoach, oldValues } = req.body;
    const {Class_ID,Coach_ID,No_of_seats,Train_ID} = selectedCoach;
    // Check if coach already exists for the current train ID
    const checkCoachQuery = 'SELECT * FROM train_coach WHERE Train_ID = ? AND Coach_ID = ?';
    db.query(checkCoachQuery, [Train_ID, Coach_ID], (checkError, checkResults) => {
      if (checkError) {
        console.error('Error checking coach existence:', checkError);
        return res.status(500).json({ error: 'Internal server error' });
      }
  
      if (checkResults.length === 0) {
        const updateCoachQuery = 'UPDATE train_coach SET Coach_ID =?,No_of_seats=?,Class_ID=? WHERE train_coach_id = ?';
        db.query(updateCoachQuery, [Coach_ID, No_of_seats,Class_ID,oldValues.train_coach_id], (updateError, updateResults) => {
          if (updateError) {
            console.error('Error updating coach details:', updateError);
            return res.status(500).json({ error: 'Internal server error' });
          }
          return res.status(200).json({ message: 'Coach details updated successfully' });
        });
     }else{
         return res.status(200).json({message: 'Coach Already Exist!'})
     }
    });
  });
app.post('/api/toggleStationStatus',(req,res)=>{
    const Station_ID = req.body.Station_ID;
    const updateStatus = req.body.updateStatus;

    const statusSql = "UPDATE station SET station_status=? where Station_ID=?";
    db.query(statusSql, [updateStatus, Station_ID], (err, result) => {
        if(err){
            res.status(500).json({ error: err})
            console.log(err)
        }
...
app.post("/api/getStation", (req, res) => {
    ...
    const sqlSelectStation = "SELECT * FROM station"
...
app.post("/api/findClerk", (req, res) => {
    const { clerk_ID, name, mobile } = req.body;

    let sqlFindClerk = "SELECT *,booking_clerk.Name as clerk_name,station.name as stationName FROM booking_clerk inner join station on booking_clerk.Station_ID = station.Station_ID WHERE 1 = 1";
    let params = [];

    if (clerk_ID) {
        sqlFindClerk += " AND booking_clerk.Clerk_ID = ?";
        params.push(clerk_ID);
    }

    if (name) {
        sqlFindClerk += " OR booking_clerk.Name LIKE ?";
        params.push('%' + name + '%');
    }

    if (mobile) {
        sqlFindClerk += " OR booking_clerk.Mobile = ?";
        params.push(mobile);
    }


    db.query(sqlFindClerk, params, (err, result) => {
        if (err) {
            console.error("Error finding clerk:", err);
...
app.post("/api/addNewTrain", (req, res) => {
    ...
    const sqlCheck = "SELECT * FROM train where Name =? and No_of_coaches = ?";
    db.query(sqlCheck,[trainName,noOfCoaches,noOfClasses],(error,result)=>{
        if(error){
            throw error
        }
        // console.log(result)
...
app.get("/api/getTrainClass", (req, res) => {
    
    const sqlInsertPassenger = "SELECT * FROM train_class";
    db.query(sqlInsertPassenger, (err, result) => {
...
app.post("/api/addNewTrainCoach", (req, res) => {
    ...
    db.query(sqlInsertPassenger, [trainName], (err, result) => {
        ...
        console.log({coachID,classID,noOfSeats})
        db.query(sqlCommand, [trainID, coachID, classID, noOfSeats], (err1,result1) => {
            console.log(result1);
...
app.post("/api/addTrainStation", (req, res) => {
    ...
    const sqlCheck = "SELECT * FROM train_stations where Train_ID=? and Station_ID=? and Up_time=? and Down_time=? and Position=?"  
    const sqlInsertTrainStation =
      "INSERT INTO train_station (Train_ID, Station_ID, Up_time, Down_time, Position) VALUES (?,?,?,?,?)";
    db.query(sqlCheck,[trainID, stationID, selectedUpTime, selectedDownTime, position],(error,CheckResult)=>{
        ...
        db.query(
            sqlInsertTrainStation,
            [trainID, stationID, selectedUpTime, selectedDownTime, position],
            (err,result) => {
              if(err) {
                  console.log(err);
...
app.post('/api/getCoachDetailsByTrainID',(req,res)=>{
    const Train_ID = req.body.Train_ID;
    const selectQuery = "SELECT train_coach.train_coach_id,train_coach.Train_ID,train_coach.Coach_ID,train_coach.No_of_seats,train_coach.coach_status,train_coach.Class_ID,train_class.class_name from train_coach inner join train_class on train_coach.Class_ID = train_class.class_id where train_coach.Train_ID=?";

    db.query(selectQuery, [Train_ID],function(err,result){
        
        if(err) return console.log(err);

        if(result.length > 0 ) {
            return res.status(200).json({trainCoachData: result});
        }
    })
},[])


app.post("/api/addTrainFare", (req, res) => {
    ...
    const sqlCheck = "SELECT * fare train_stations where Train_ID=? and Start_=? and Up_time=? and Down_time=? and Position=?"  
    const sqlInsertTrainStation =
      "INSERT INTO train_station (Train_ID, Station_ID, Up_time, Down_time, Position) VALUES (?,?,?,?,?)";
    db.query(sqlCheck,[trainID, stationID, selectedUpTime, selectedDownTime, position],(error,CheckResult)=>{
...
app.get('/api/getClerk', (req, res) => {

    const sqlSelectClerk = "SELECT *, booking_clerk.Name, station.Name as stationName FROM booking_clerk inner join station on booking_clerk.Station_ID = station.Station_ID"
    db.query(sqlSelectClerk,(err, result) => {
        if(err) throw err;
